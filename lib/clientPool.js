...
function register(socket, clientPool) {
  ...
  socket.on('close', () =>  {
    // console.log(socket.profile.id + ' disconnecting');
    console.log(Object.keys(clientPool.pool));
    clientPool.ee.emit('disconnect', socket);
    console.log(Object.keys(clientPool.pool));
...
const ClientPool = module.exports = function() {
  ...
  this.ee.on('register', (socket) => {
    ...
    Object.keys(this.pool).forEach(function(clientid) {
      if(socket.profile.id !== clientid)
        // others
        this.pool[clientid].write(formatMessage('serverMessage', chalk[socket.profile.color](socket.profile.nickname) + ' connected to server\n'));
      else
        // self
  ...
  this.ee.on('broadcast', (socket, data) => {
    ...
    Object.keys(this.pool).forEach(function(clientid) {
      if(socket.profile.id !== clientid)
        this.pool[clientid].write(formatMessage('userMessage', message, socket));
    }.bind(this));
  });

  this.ee.on('nickChange', (socket, data) => {
    var newNick = data.toString().slice(5).trim();
    var message = chalk[socket.profile.color](socket.profile.nickname) + ' is changing nickname to ' + chalk[socket.profile.color](newNick);
    console.log(message);
    Object.keys(this.pool).forEach(function(clientid) {
      if(socket.profile.id !== clientid)
        this.pool[clientid].write(formatMessage('serverMessage', message));
    }.bind(this));
    socket.profile.nickname = newNick;
  });

  this.ee.on('disconnect', (socket) => {
    let message = chalk[socket.profile.color](socket.profile.nickname) + ' has disconnected from the server\r\n';
    console.log(message);
    Object.keys(this.pool).forEach(function(clientid) {
      this.pool[clientid].write(formatMessage('serverMessage', message));
    }.bind(this));
...
function formatMessage(messageType, message, socket) {
  if (messageType === 'userMessage') {
    ...
    return `${chalk.white.bgGreen.bold('SERVER:')} ${message}`;
  }
}
